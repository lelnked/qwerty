...
export default function Translation({ trans, showTrans = true, onMouseEnter, onMouseLeave }: TranslationProps) {
  ...
  return (
    <div className={`flex items-center justify-center pb-10 pt-2`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <span
        className={`max-w-4xl text-center font-sans leading-relaxed text-gray-800 transition-colors duration-300 dark:text-white dark:text-opacity-80 ${
          isShowTransRead && 'pl-8'
        } ${isTextSelectable && 'select-text'}`}
        style={{ fontSize: Math.max(fontSizeConfig.translateFont, 26).toString() + 'px' }}
      >
        {showTrans ? trans : '\u00A0'}
      </span>
